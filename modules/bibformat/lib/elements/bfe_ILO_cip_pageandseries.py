...
def format_element(bfo, prefix, suffix):
    ...
    endpage = bfo.field('300%%a')    

    if len(endpage) > 0 and endpage != '1 v.':
        endpage = re.sub(' :$', '', endpage)
        endpage = re.sub('^1 v.$', '', endpage)
    else:
        endpage = ''
    ...
    # put page and series data together and return
    
    pageandseries = endpage + series_data

    if len(pageandseries) > 0:
        return pageandseries
    else:
        return ''
